...
11. Step 3 固定抽取 4,000 条训练导向样本：3,000 条 Step 2 正例和 1,000 条 `S→OTHER` 难负例；抽样组内部按输入年份均衡。
...
flowchart TB
    ...
    HUMAN --> RESULTS["results.csv：human_evaluation=true/false"]
...
6. 材料不足或证据冲突时仍作二选一，并设置 review_flag=true。
...
Step 3 固定输入 4,000 条。正式人工标注、仲裁和质量检查完成后的 4,000 条记录构成 Step 3
输出。开发期 Codex 标注使用相同 4,000 条输入和相同输出 Schema，但只能产生 provisional 数据，
不能直接替代人工输出。

### 8.2 3,000 正例与 1,000 难负例的稳定抽样
...
| `hard_negative` | 1,000 | `step1_route=S AND step2_label=OTHER` | 学习含安全近邻词但最终不属于数据安全的困难边界 |

负例不从 `E→OTHER` 中优先抽取。该类通常是没有有效关键词且 Step 2 也判负的容易负例，训练价值
低于 `S→OTHER`；将有限人工预算用于难负例，更有助于降低关键词触发型误报。如果
`S→OTHER` 总量不足 1,000，程序必须失败并要求修订方法版本，不得静默用 E 负例补齐。

设输入包含 `Y` 个年份，抽样组 `g` 在年份 `y` 的容量为 `N(y,g)`，组目标分别为
`Q(positive)=3000`、`Q(hard_negative)=1000`。每个组独立进行容量约束的等额“注水”分配：
...
因此，输入十年且每个单元容量充足时，每年抽取约 300 个正例和 100 个难负例，共约 400 条。
某一年度容量不足时，先取尽该单元，再只在同一抽样组的其他年份中均衡重分配；不会在正负组
之间挪用配额。若正例总量不足 3,000 或难负例总量不足 1,000，则整个抽样失败。
...
相同输入、版本和 seed 必须得到完全相同的样本。Step 2 的 `confidence`、`review_flag`、模型
...
训练时可以直接使用 3:1 的构成或另行设置损失权重；估计原始专利总体比例、precision、recall
或其他总体指标时，必须使用两阶段 `w_combined`，不得把 3:1 样本当作自然发生率。

### 8.3 正式人工标注与质量控制

首轮人工标注文件只展示 `sample_id`、年份、专利名称、摘要、主权项和 IPC，不展示 Step 1
路由、Step 2 标签、置信度、解释、模型名和抽样组。这样可以避免把“审核”变成对既有模型结论
的默认确认。

每条专利至少由一名标注员独立完成与 Step 2 兼容的二分类和证据字段：

```text
label, confidence, scope_basis, processing_activities, industry_sectors,
technical_scope, legal_scope, evidence, reason, review_flag, review_reason
```

正式流程的质量控制为：

1. 从 4,000 条中按“年份×抽样组”再稳定抽取 400 条（10%）进行双人独立标注；
2. 双标样本报告原始一致率、正负类分别一致率和 Cohen's kappa；
3. 双标不一致、任一标注员设置 `review_flag=true`、证据无法回指原文，或人工标签与 Step 2
   标签不一致的样本全部进入仲裁；
4. 仲裁员只能在两名首轮标注完成后查看既有结论，并记录最终标签、理由、仲裁人和时间；
5. 未完成必需字段、存在重复专利、文本不可读或仍有未解决冲突的记录不得进入 Gold 输出。

Step 2 标签可以用于抽样和标注后的差异审计，但不得在首轮人工判断前展示。
...
Step 3 的版本目录按 `sample/`、`state/`、`dataset/` 三类用途组织，至少输出：

| 文件 | 内容 |
| --- | --- |
| `sample/audit.csv` | Step 1/2 结论、抽样层、概率、权重、模型与文本，禁止给首轮标注员 |
| `sample/annotation_input.csv` | 4,000 条盲化人工输入模板 |
| `sample/manifest.json` | 输入哈希、配额、来源和实际抽样分层 |
| `state/tasks.sqlite3` / `state/progress.json` | Codex 模拟任务、状态、重试、结果、调用审计和进度 |
| `dataset/simulation.csv` | Codex 模拟明细；只用于复核辅助，不是训练输入 |
| `dataset/results.csv` | 4,000 条正式人工结果；唯一切分输入 |
| `dataset/splits/train.csv` | 由 `results.csv` 生成的训练集 |
| `dataset/splits/validation.csv` | 由 `results.csv` 生成的验证集 |
| `dataset/splits/test.csv` | 由 `results.csv` 生成的测试集 |
| `dataset/split_report.json` | 切分与质量报告 |

`results.csv` 及三个切分文件严格只保留以下字段，前序模型、置信度、理由、证据、复核标记、
抽样概率和权重等列全部删除：
...
`human_evaluation=true` 映射为 `DATA_SECURITY`，`false` 映射为 `OTHER`；两个分类维度使用 JSON
数组。数据切分只在 4,000 条人工结果完整、唯一且与冻结盲标原文逐字段一致后执行：
...
Step 3 已冻结的 3,200/400/400 人工切分，不重新随机划分：

| 集合 | 总数 | 用途 |
| --- | ---: | --- |
| train | 3,200 | 参数训练 |
| validation | 400 | 按 accuracy 选择 checkpoint |
| test | 400 | 完成全部选择后的单次最终评估 |

Step 4 只接受 `dataset/results.csv` 及其人工切分，逐行从 `human_evaluation` 派生模型标签。
`simulation.csv` 以及 Codex 的模型、理由、置信度和复核字段均不得进入 Step 4。

数据准备必须重新验证：`results.csv` 与三个集合逐字段一致、三个集合的精确数量、标签取值、
...
`dataset/manifest.json` 保存人工 `results.csv`、全部切分和输出文件 SHA-256、标签数与映射；
...
生产运行默认保持 10 个方舟请求并发。终端和 Step 2 方法版本目录下的 `<dataset>/progress.json` 必须持续输出或保存：完成数、成功数、失败数、完成比例、本次运行墙钟耗时、所有请求累计耗时、平均请求耗时、已完成任务平均耗时、预计剩余秒数和预计完成时间。ETA 沿用 V1 的可解释估计：
...
11. 人工 `results.csv` 与切分只保留规定的 12 列，最终全局切分严格为 3,200/400/400，且完全相同文本不跨数据集；
12. 测试集冻结后不参与任何规则、Prompt、分类器选择或模型迭代。

### 12.4 Step 4

1. 只接受已经冻结且精确为 3,200/400/400 的 Step 3 split；
2. 三个 split 必须与人工 `results.csv` 逐字段一致，全局无重复 `sample_id`、`patent_id`，完全相同文本不跨集合；
3. 只从 `human_evaluation=true/false` 派生模型标签，拒绝 Codex simulation 输入；

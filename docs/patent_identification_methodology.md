...
Step 2 的 `confidence`、`review_flag`、模型
...
| `annotation_increment.csv` | 从旧版本扩容时新增的 1,000 条盲标输入，不展示 Step 1/2 结论 |
...
human_evaluation,confidence,scope_basis,processing_activities,industry_sectors,
technical_scope,legal_scope,evidence,reason,review_flag,review_reason
```
...
`human_evaluation` 派生模型标签。Step 4 只接受 `finalize` 已在 manifest 中登记哈希的完整结果，

...
- 文档状态：2009—2025 年 Step 1 已完成；跨年份 50,000 条 Step 2 任务池已生成；模型识别待命令执行
- 版本：2.4.0-draft
- 更新日期：2026-07-24
...
13. 正式人工标签与开发期 Codex 模拟标签严格区分；只有完成独立人工标注和必要仲裁的数据才能称为 Gold。
...
11. 两批人工结果经严格去重和冻结字段校验后生成 `result.csv`，最终全局切分严格为 8,000/1,000/1,000，且完全相同文本不跨数据集；
12. `evaluate` 同时报告样本指标和面向完整 50,000 条 Step 2 任务池的设计加权指标；该口径不能外推到未进入 Step 2 的原始专利；

...
抽样分布、断点续跑、SFT 格式和切分程序。实现使用本机 `codex exec --ephemeral`，不读取或要求
...
它可用于开发期 BERT/SFT 试验和发现标注规范问题，但不得用于报告最终模型性能，也不得与人工
Gold 合并后隐藏来源。当前模型名称和 Codex 调用能力以运行时记录为准；相关产品用法参考
...
| `dataset/provisional.csv` | 开发期合并训练字段与来源状态；只保留一份全量数据 |
| `dataset/splits/train.csv` | 训练集 |
| `dataset/splits/validation.csv` | 验证集 |
| `dataset/splits/test.csv` | 测试集 |
| `dataset/split_report.json` | 切分与质量报告 |

数据切分只在获得当前阶段的最终标签后执行。正式人工数据使用仲裁后的人工标签；开发期使用
Codex 模拟标签，但输出文件名和状态必须明确为 provisional。4,000 条严格切分为：

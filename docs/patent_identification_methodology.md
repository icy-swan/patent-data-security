...
11. Step 3 固定两个互不重叠的 5,000 条队列：正向优先队列按 Step 2 预测正/负 3:2，负向优先队列按 2:3 且优先覆盖剩余 `S→OTHER` 难负例；合并后共 10,000 条。
...
`patent_id`。在当前 2021 年 Step 2 任务池中，第一队列已经使用 2,000 条难负例，剩余难负例

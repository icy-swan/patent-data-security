...
- 文档状态：2009—2025 年 Step 1 与 50,000 条 Step 2 识别已完成；Step 3 已抽取 10,000 条待复核样本
- 版本：2.6.0-draft
- 更新日期：2026-07-25
- 适用范围：中国上市公司专利文本的数据安全领域识别
- 本版范围：Step 1 关键词路由、Step 2 大模型识别，以及 Step 3 人工标注抽样、训练集切分、开发期 Codex 模拟标注与 Kimi K3 Agent Plan 测试复核
...
4. 全部唯一 `S` 与按稳定概率抽取的唯一 `E` 共同组成 Step 2 候选框；
5. 本轮把 2009—2025 年候选合并后再次按 `patent_id` 全局去重，并以固定 seed 等概率无放回抽取 50,000 件进入实际 Step 2 请求池；
6. Step 2 做宽口径的领域归属二分类，不再使用旧版“发明中心性闭环”作为正类门槛；
7. Step 2 不接收 Step 1 的路由、命中词、抽样概率或上下文诊断，避免关键词锚定模型结论；
8. 《中华人民共和国数据安全法》全文进入 Step 2 的固定 Prompt 前缀；每件专利只作为动态后缀；
9. 固定前缀通过上下文缓存复用，专利之间不串联会话；
10. v1 的请求客户端、严格 JSON 校验、SQLite 断点续跑、重试、并发、进度记录和原子导出机制继续复用，但代码按 `pipeline/step1`、`pipeline/step2`、`pipeline/step3` 重组。
11. `DATA_SECURITY` 专利同时标注“数据处理环节”和“行业领域”两个受控多标签维度，供后续数据分析使用；这两个维度不改变主标签。
12. Step 3 从冻结的 50,000 条 Step 2 任务池中固定抽取两个互不重叠的 5,000 条队列：正向优先队列按 Step 2 预测正/负 3:2，负向优先队列按 2:3 并固定包含 1,000 条 `S→OTHER` 难负例和 2,000 条 `E→OTHER` 容易负例；合并后共 10,000 条。
13. 正式人工标签与 Codex、Kimi K3 等模型测试标签严格区分；只有完成独立人工标注和必要仲裁的数据才能称为 Gold。
...
flowchart TB
    ...
    SA --> Q["跨年份 Step 2 候选框"]
    ES --> Q
    Q --> GDEDUP["按 patent_id 全局去重"]
    GDEDUP --> SAMPLE["固定 seed 等概率无放回抽取 50,000 件"]
    SAMPLE --> REQUESTS["Step 2 实际请求池"]
    REQUESTS --> P["固定前缀：判定规则 + 数据安全法全文 + Schema"]
    REQUESTS --> X["动态后缀：单件专利材料"]
    ...
    DS --> POS["Step 3：两个队列合计抽取 5,000 个预测正例"]
    O --> HARD["Step 1=S 的 OTHER 难负例候选"]
    O --> EASY["Step 1=E 的 OTHER 容易负例候选"]
    HARD --> NEG["两个队列合计抽取 3,000 个难负例"]
    EASY --> NEG2["负向优先队列抽取 2,000 个容易负例"]
    POS --> REVIEW_INPUT["双队列复核输入：共 10,000 条"]
    NEG --> REVIEW_INPUT
    NEG2 --> REVIEW_INPUT
    REVIEW_INPUT --> HUMAN["正式人工复核与仲裁"]
    REVIEW_INPUT --> K3["Kimi K3 Agent Plan 测试复核：非 Gold"]
    POS --> CODEX["开发期 Codex 独立模拟标注：非 Gold"]
    HUMAN --> RESULTS["result.csv：human_review_label=DATA_SECURITY/OTHER"]
    CODEX --> SIMULATION["simulation.csv：只用于复核辅助"]
    K3 --> K3_RESULT["k3_result.csv：k3_review_label + k3_reason"]
...
各阶段不再嵌套方法版本目录。Step 1 以输入数据集年份隔离；本轮 Step 2 使用跨年份固定任务池，
Step 3 和 Step 4 使用固定根目录。版本、哈希和统计信息统一保存在 manifest 中：
...
│   ├── manifest.json
│   ├── tasks.sqlite3
│   ├── need_manual_review_{positive,negative}.csv
│   ├── simulation.csv             # 可选 Codex 模拟
│   ├── k3_tasks.sqlite3           # 可选 K3 测试复核
│   ├── k3_manifest.json
│   ├── k3_progress.json
│   ├── k3_result.csv
│   ├── result.csv                 # 人工标注后提供
...
保留 `progress.json`，迁移时不补造过程记录。Step 3 的 Codex 与 K3 模型测试结果都不是人工
真值，不能代替 `result.csv`。
...
全部唯一 `S` 与按稳定概率抽取的唯一 `E` 先进入 Step 2 候选框；唯一 `E` 按
`patent_id + dataset_id + seed` 做稳定哈希抽样。
...
2009—2025 年 Step 1 先形成候选框：

```text
candidate_frame = global_deduplicate(
    all(unique S) + stable_sample(unique E, p_E),
    key=patent_id
)
step2_pool = first_n(
    sort_by_sha256("step2-global-pool-v1" + "|" + patent_id),
    n=50000
)
```

本轮 Step 1 共给出 224,907 条候选记录；跨年份按 `patent_id` 去重后为 224,906 件唯一专利，
其中只有 1 条跨年份重复记录。固定抽取结果为 `S_all` 21,711 件、`E_random` 28,289 件，
合计 50,000 件。因此每件候选专利的二次纳入概率为：

```text
q_pool = 50000 / 224906
combined_selection_probability = step1_selection_probability * q_pool
sample_weight = 1 / combined_selection_probability
```

抽样不强制年份、`S/E` 或抽样组配额；这些属性只用于抽样后的分布审计。因此，候选框内每件
唯一专利具有相同的二次纳入概率。全局去重时保留字典序最早的 `dataset_id`，各年份内部继续使用
Step 1 已选择的规范关联行。同一专利出现在多家公司或多个年份时只请求一次，公司关联关系在
模型完成后再回连，避免重复成本和重复样本权重。

任务准备命令会生成 50,000 行 `requests.jsonl` 和对应的 `tasks.sqlite3`，只做本地读取、绑定、
哈希与抽样，`llm_requests_executed` 必须为 0。`requests.jsonl` 是一行一件专利的离线审计
副本，不作为一次性批量请求体。实际运行时，runner 每次从 SQLite 领取一件任务，独立构造一次
“固定前缀 + 当前专利动态后缀”的 Responses 请求；并发只表示多个单件请求可以同时在途，请求
之间不共享或累积会话上下文。只有收到单独的识别命令后才能启动模型请求。
...
Step 3 固定为两个互不重叠的 5,000 条人工复核队列，合并规模为 10,000 条。两个队列分别服务于
“正例较多”和“负例较多”的训练实验；模型间的正式比较仍必须共用同一份冻结测试集。开发期
Codex 标注只能产生 provisional 数据，不能替代人工输出。

### 8.2 正向优先与负向优先双队列稳定抽样

队列名称表示抽样倾向，不表示人工真实标签。配额均以抽样时冻结的 `step2_label` 计算：

| 队列 | Step 2 预测正例 | Step 2 预测负例 | 负例选择 |
| --- | ---: | ---: | --- |
| `positive_priority` | 3,000 | 2,000 | `step1_label=DATA_SECURITY AND step2_label=OTHER` 难负例 |
| `negative_priority` | 2,000 | 3,000 | 1,000 条难负例 + 2,000 条 `step1_label=OTHER AND step2_label=OTHER` 容易负例 |
| 合并 | 5,000 | 5,000 | 3,000 条难负例 + 2,000 条容易负例 |

代码只接受冻结的 50,000 条 Step 2 任务池。当前任务池包含 14,631 条 Step 2 正例、7,324 条
`S→OTHER` 难负例和 28,045 条 `E→OTHER` 容易负例。第二队列严格排除第一队列全部
`sample_id` 和 `patent_id`，并固定抽取 2,000 条尚未入样的 Step 2 正例、1,000 条尚未入样的
难负例和 2,000 条容易负例。合并后为 5,000 个 Step 2 正预测与 5,000 个 Step 2 负预测，
其中负预测由 3,000 条难负例和 2,000 条容易负例构成。固定保留容易负例，使三个抽样组均有
非零纳入概率；若第二队列继续“先抽完难负例”，当前任务池中的容易负例将完全无法进入样本，
设计加权结果也不能推广到完整 Step 2 任务池。

这个 2:3 只能保证第二队列的 Step 2 预测标签比例，不能保证人工 Gold 比例。困难负例可能被
人工改判为正例，因此第二队列和合并数据的真实正负数必须在人工复核后按
`human_review_label` 重新统计，不得在复核前宣称 Gold 已平衡。

每个队列的各抽样组均在输入年份间做容量约束的等额“注水”分配。设剩余候选容量为
`N_remaining(y,g)`、队列组目标为 `Q(g)`：

```text
sum_y n(y,g) = Q(g)
0 <= n(y,g) <= N_remaining(y,g)
在未达到容量上限的年份之间，使 n(y,g) 尽可能相等
```

每个“年份×抽样组”内部按固定 seed 的 SHA256 排序，保证相同输入、版本和 seed 得到相同样本：

```text
score = SHA256(sample_seed | dataset_id | patent_id)
```

Step 2 的 `confidence`、`needs_review`、解释和请求耗时不参与排序。`manifest.json` 同时保存各队列
配额、实际构成、来源哈希、条件抽样层，以及合并样本在完整 Step 2 任务池中的“年份×抽样组”
纳入概率。训练可以选择第一队列、第二队列或二者合并；总体指标必须使用设计权重，不能把人为
配平后的样本比例当作自然发生率。

### 8.3 正式人工复核与质量控制

本轮改为对 Step 2 结论的全量人工复核，不再称为盲标。`need_manual_review_positive.csv` 与
`need_manual_review_negative.csv` 展示
`sample_id`、年份、专利名称、摘要、主权项、IPC，以及 Step 2 的标签、置信度、受控维度、
技术/法律范围、逐字证据、判断理由和不确定性提示，同时明确保存 `step1_label` 与
`sample_cohort`。人工必须填写：
...
1. 从 10,000 条中按“队列×年份×抽样组”稳定抽取 1,000 条（10%）进行双人独立标注；
...
### 8.5 Kimi K3 Agent Plan 测试复核

为比较另一模型对同一待复核材料的判断，Step 3 提供独立的 Kimi K3 测试链路。该链路固定使用
模型名 `kimi-k3`，认证只读取 `.env` 中的 `ARK_API_KEY`，同时要求
`ARK_API_KEY_KIND=agent-plan`，并校验数据面地址严格为
`https://ark.cn-beijing.volces.com/api/plan/v3`。API Key 不进入命令行、日志、CSV 或
manifest。
...
`k3_review_label` 只能是 `DATA_SECURITY` 或 `OTHER`；`k3_reason` 必须说明支持结论的正文技术
特征以及维持或修改 Step 2 标签的原因。两列追加到冻结的 24 列人工复核输入末尾，10,000 条
全部成功后才原子生成 `k3_result.csv`。

K3 使用独立的 `k3_tasks.sqlite3`、`k3_manifest.json`、`k3_progress.json`、runner 日志和锁，
不会修改 Codex 的 `tasks.sqlite3`、`progress.json`、`simulation.csv`，也不会填写
`human_review_label` 或 `human_reason`。运行中断后只恢复未成功的单件任务；模型、Prompt、
Schema 或输入文件哈希变化时拒绝混跑。K3 输出固定标记为 provisional model review，不进入
`merge`、`finalize`、训练集或正式评估。
...
| `tasks.sqlite3` | 10,000 条冻结正文和唯一身份绑定 |
| `need_manual_review_positive.csv` | 第一批 5,000 条正向优先人工复核输入 |
| `result_positive.csv` | 第一批 5,000 条已复核结果 |
| `need_manual_review_negative.csv` | 第二批 5,000 条负向优先人工复核输入 |
| `result_negative.csv` | 第二批 5,000 条已复核结果 |
| `k3_tasks.sqlite3` | 可选 K3 测试复核的独立断点续跑状态 |
| `k3_manifest.json` | K3 输入哈希、模型、调用粒度和输出契约 |
| `k3_progress.json` | K3 请求进度、失败数、耗时和 token 用量 |
| `k3_result.csv` | 10,000 条全部成功后生成的 K3 测试复核结果；非 Gold |
| `result.csv` | 两批结果通过严格合并校验后的 10,000 条 Gold；唯一切分输入 |
| `dataset/train.csv` | 由 `result.csv` 生成的训练集 |
| `dataset/validation.csv` | 由 `result.csv` 生成的验证集 |
| `dataset/test.csv` | 由 `result.csv` 生成的测试集 |

`python -m pipeline.step3 merge` 校验两个结果文件的队列标识、行数、必填标签和理由、冻结字段、
样本 ID 与专利 ID。只有两批各 5,000 条且批内/批间均无重复时才原子生成 `result.csv`；输入、
输出哈希与各队列 Gold 标签统计写入 manifest。`progress.json` 与 `simulation.csv` 仅在显式执行
可选 Codex 模拟时生成；所有 `k3_*` 文件仅由显式的 K3 准备或运行命令生成。
...
人工复审标签分层切分，不再执行布尔翻转。数据切分只在 10,000 条人工结果
...
8,000/1,000/1,000。若未来补充专利族 ID，应以专利族作为更高优先级的切分组，防止同族泄漏。
...
Matthews correlation coefficient、Cohen's Kappa。10,000 条样本的 Accuracy 另报告 Wilson 95%
置信区间。
...
| 口径 | 含义 | 论文使用限制 |
| --- | --- | --- |
| `sample_unweighted` | 10,000 条双队列定额样本上的直接结果 | 只能描述该样本，不是总体准确率 |
| `eligible_frame_design_weighted` | 按“年份×抽样组”的 Step 3 纳入概率倒数加权 | 推广至完整 Step 2 任务池 |

双队列合并后包含 Step 2 正例、`S→OTHER` 难负例和 `E→OTHER` 容易负例，设计加权结果可推广到
完整 Step 2 任务池，但不能仅凭 Step 3 权重推广到进入 Step 2 之前已被排除的全部原始专利。
尤其 Step 1 的 `S/E` 原本用于高召回路由，不能把其诊断 Accuracy 当作最终分类器性能。
...
Step 4 使用 Step 3 两个队列合并并完成人工核验后的 10,000 条数据。输入直接使用
Step 3 已冻结的 8,000/1,000/1,000 人工切分，不重新随机划分：

| 集合 | 总数 | 用途 |
| --- | ---: | --- |
| train | 8,000 | 参数训练；可按 `sample_cohort` 构造正向优先、负向优先和合并三种训练版本 |
| validation | 1,000 | 按 accuracy 选择 checkpoint；各训练版本共用 |
| test | 1,000 | 完成全部选择后的单次最终评估；各训练版本共用 |
...
```text
data/step4/
├── archive/data-security-binary-v1.0.0/  # 历史实验的 4,000 条简化 Prompt 基线，非当前切分
├── dataset/
│   ├── manifest.json
...
8. Step 2 候选框改为全部唯一 S 加稳定 E 样本；当前生产批次再做跨年份全局去重与固定 50,000 件等概率抽样。
...
`pipeline/step3/README.md` 给出双 5,000 条队列冻结、人工结果合并、切分和评估命令。
...
13. 候选框为 224,906 件全局唯一专利，固定 seed 无放回抽样必须精确产生 50,000 件；
14. `requests.jsonl` 只能包含 `patent_id/title/abstract/claim/ipc/main_ipc`，不得泄露 Step 1 路由、命中词或抽样字段；
15. `manifest.json` 保存候选数、跨年份重复数、固定 seed、二次纳入概率、来源哈希和年份/路由分布，且准备阶段的模型请求数为 0。

生产运行默认保持 10 个方舟请求并发。终端和
`data/step2/progress.json` 必须持续输出或保存：完成数、成功数、失败数、完成比例、
本次运行墙钟耗时、所有请求累计耗时、平均请求耗时、已完成任务平均耗时、预计剩余秒数和预计
完成时间。ETA 沿用 V1 的可解释估计：
...
1. 固定产生两个各 5,000 条且互不重叠的队列，合并为 10,000 条唯一专利样本；
2. 第一队列严格为 3,000 条 Step 2 `DATA_SECURITY` 和 2,000 条 `S→OTHER` 难负例；第二队列严格为 2,000 条 Step 2 `DATA_SECURITY`、1,000 条 `S→OTHER` 难负例和 2,000 条 `E→OTHER` 容易负例；
3. 任一抽样组容量不足时失败，不从其他组静默补齐；
4. 多年份输入在每个抽样组内部进行容量约束的均衡分配；
5. 相同输入、sampling version 和 seed 可复现完全相同的 `sample_id` 集合；
6. 冻结正文保存在任务库中；两个 `need_manual_review_{positive,negative}.csv` 展示 Step 1/2 标签、抽样队列、Step 2 置信度、证据和理由；
...
10. K3 测试固定使用 Agent Plan 的 `kimi-k3`，严格一件专利一次请求，独立状态库断点续跑；
11. `k3_result.csv` 只追加 `k3_review_label`、`k3_reason`，不得填写人工字段或进入训练和正式评估；
12. 正式 Gold 对 10%（1,000 条）样本双标，并仲裁全部规定冲突；
13. 两批人工结果经严格去重和冻结字段校验后生成 `result.csv`，最终全局切分严格为 8,000/1,000/1,000，且完全相同文本不跨数据集；
14. `evaluate` 同时报告样本指标和面向完整 50,000 条 Step 2 任务池的设计加权指标；该口径不能外推到未进入 Step 2 的原始专利；
15. 测试集冻结后不参与任何规则、Prompt、分类器选择或模型迭代。

### 12.4 Step 4

1. 只接受已经冻结且精确为 8,000/1,000/1,000 的 Step 3 split；
2. 三个 split 必须与人工 `result.csv` 逐字段一致，全局无重复 `sample_id`、`patent_id`，完全相同文本不跨集合；
3. 只从显式 `human_review_label=DATA_SECURITY/OTHER` 读取模型标签，拒绝 Codex simulation 与 K3 review 输入；

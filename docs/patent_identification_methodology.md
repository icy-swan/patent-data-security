...
| 文件 | 内容 |
| --- | --- |
| `step3_sample_audit.csv` | Step 1/2 结论、抽样层、概率、权重、模型与文本，禁止给首轮标注员 |
| `step3_annotation_input_blinded.csv` | 4,000 条盲化人工输入模板 |
| `step3_tasks.sqlite3` | Codex 模拟任务、状态、重试、结果和调用审计 |
| `step3_dataset_<status>.csv` | 合并后的训练字段与来源状态 |
| `step3_train_<status>.csv` | 训练集 |
| `step3_validation_<status>.csv` | 验证集 |
| `step3_test_<status>.csv` | 测试集 |
| `step3_sample_manifest.json` / `step3_split_report.json` | 输入哈希、配额、实际分层、切分与质量报告 |
...
生产运行默认保持 10 个方舟请求并发。终端和 `step2_progress_<dataset>.json` 必须持续输出或保存：完成数、成功数、失败数、完成比例、本次运行墙钟耗时、所有请求累计耗时、平均请求耗时、已完成任务平均耗时、预计剩余秒数和预计完成时间。ETA 沿用 V1 的可解释估计：

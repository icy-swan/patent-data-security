...
对于 OpenAI 官方端点，[Prompt caching 官方最佳实践](https://developers.openai.com/api/docs/guides/prompt-caching#best-practices)要求把静态重复内容放在开头、动态内容放在结尾，并持续记录 `cached_tokens` 和 `cache_write_tokens`；支持时可对相同长前缀使用稳定的 `prompt_cache_key`。第三方 OpenAI 兼容端点未必接受该参数，因此实现默认使用字节稳定前缀，只有显式启用时才发送 `prompt_cache_key`，并以实际 usage 返回值判断是否命中，不能因“兼容 Responses API”就推定缓存能力相同。

...
12. Step 3 固定两个互不重叠的 5,000 条队列：正向优先队列按 Step 2 预测正/负 3:2，负向优先队列按 2:3 且优先覆盖剩余 `S→OTHER` 难负例；合并后共 10,000 条。
...
| `negative_priority` | 2,000 | 3,000 | 先抽完尚未入样的难负例，再用 `step1_label=OTHER AND step2_label=OTHER` 容易负例补足 |
| 合并 | 5,000 | 5,000 | 难负例与容易负例均有覆盖 |

第一队列保留已经完成的 5,000 条人工工作。第二队列严格排除第一队列全部 `sample_id` 和
`patent_id`。在历史 2021 年 Step 2 任务池中，第一队列已经使用 2,000 条难负例，剩余难负例
1,038 条，因此第二队列由 2,000 条尚未入样的 Step 2 正例、1,038 条剩余难负例和 1,962 条
尚未入样的容易负例构成。合并后为 5,000 个 Step 2 正预测与 5,000 个 Step 2 负预测。
...
python -m pipeline.step2 run --dataset-id pool-50000 ...
python -m pipeline.step2 status --dataset-id pool-50000 ...
python -m pipeline.step2 stop --dataset-id pool-50000 ...
...
`data/step2/pool-50000/progress.json` 必须持续输出或保存：完成数、成功数、失败数、完成比例、

...
当前 Step 3 实现版本 `step3-gold-v1.0.0` 将约 2,000 条目标规模具体冻结为：代表性核心 1,500
条、风险加抽 500 条。置信度区间为 `<0.80`、`[0.80,0.90)`、`[0.90,0.98)` 和
`[0.98,1.00]`。风险加抽目标权重依次为：类别 2/`review_flag` 36%、多次请求后成功 12%、
总体频数不超过 100 的罕见 subtype 12%、高置信 S/W -> 类别 3 冲突 24%、其余
S/W/R -> 类别 3 转换 16%。风险条件重叠时按上述顺序归入唯一主风险组。

为得到可直接用于加权评价的明确设计概率，实现先计算代表性配额和风险加抽配额，再将二者合并到
`年度 × S/W/R × G0类别 × 置信度区间 × 主风险组`互斥细分层中，最后在每个细分层内执行一次
不放回简单随机抽样。因此每条入选记录的入样概率严格为 `n_h/N_h`，评价权重为 `N_h/n_h`。
抽样 seed、配额、总体数和细分层均写入 Step 3 审计产物；任何重新抽样必须提高版本或显式记录
`--rebuild`，不得静默覆盖已进入人工流程的冻结样本。

...
flowchart TB
    ...
    HUMAN --> RESULTS["result.csv：human_evaluation=true/false"]
...
| `tasks.sqlite3` / `progress.json` | Codex 模拟任务、冻结正文、状态、重试、结果、调用审计和进度 |
| `simulation.csv` | Codex 模拟明细；只用于复核辅助，不是训练输入 |
| `result.csv` | 等待人工提供的 4,000 条正式结果；唯一切分输入 |
| `dataset/train.csv` | 由 `result.csv` 生成的训练集 |
| `dataset/validation.csv` | 由 `result.csv` 生成的验证集 |
| `dataset/test.csv` | 由 `result.csv` 生成的测试集 |

`result.csv` 及三个切分文件严格只保留以下字段，前序模型、置信度、理由、证据、复核标记、
抽样概率和权重等列全部删除：
...
Step 4 只接受 Step 3 根目录 `result.csv` 及 `dataset/` 下的人工切分，逐行从
`human_evaluation` 派生模型标签。

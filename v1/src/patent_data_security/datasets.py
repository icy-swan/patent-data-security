"""Dataset discovery and collision-safe identifiers for multi-file workflows."""
...
def dataset_id(path: str | Path) -> str:
    """Return a stable output token, preferring a four-digit year in the filename."""
...
def discover_files(
    ...
    """Resolve explicit inputs or discover all matching files in a directory."""
    ...
    if not paths:
        raise FileNotFoundError(f"No input files found for pattern {pattern!r} in {directory}")
...
def _validate_unique_dataset_ids(paths: tuple[Path, ...]) -> None:
    ...
    collisions = {token: values for token, values in by_id.items() if len(values) > 1}
    if collisions:
        details = "; ".join(
            f"{token}: {', '.join(path.name for path in values)}"
            for token, values in collisions.items()
        )
        raise ValueError(f"Dataset IDs collide; use unique years or filenames: {details}")
